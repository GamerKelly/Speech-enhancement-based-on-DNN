...
if __name__ == "__main__":
    ...
    # 定义device
    device = torch.device("cuda:0")                  #原始参数存储在cuda:0
    ...
    #m_model = DataParallel(m_model, device_ids=[0, 1])   #DataParallel()表示数据并行计算,若想单卡训练，注释掉该行即可；12/09：双卡训练进行验证时会报错
    m_model = m_model.to(device)                         #两个GPU上的参数会重新传回cuda:0进行汇总和参数更行
    m_model.train()
    ...
    for epoch in range(n_epoch):
        ...
        for i_batch, sample_batch in enumerate(m_DataLoader):
            ...
            # 进行参数更新
            # optimizer.zero_grad()
            optimizer.step()         #step即完成一批数据前向传播、反向传播和参数更新给过程
            ...
            # 每100 step 输出一次中间结果
            if n_step %100 == 0:
                print("epoch = %02d  step = %04d  loss = %.5f"%(epoch,n_step,loss))
        ...
        # 进行模型保存
        save_name = os.path.join('save','model_%d_%.5f.pth'%(epoch,loss_mean))
